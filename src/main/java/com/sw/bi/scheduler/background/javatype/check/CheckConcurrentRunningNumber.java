...
//    /home/tools/scheduler/scheduler jar   /home/tools/scheduler/scheduler.jar com.sw.bi.scheduler.background.javatype.check.CheckConcurrentRunningNumber
@Component
public class CheckConcurrentRunningNumber {
	private static final Logger log = Logger.getLogger(CheckConcurrentRunningNumber.class);
	...
	public void check(final int runningNumberThreshold, final int beginTimeInterval, final int tryTimes, final int tryInterval, final String[] mobiles, final Long[] excludeJobIds) {
		...
		while (true) {
			try {
				int count = actionService.countTodayRunningActions(beginTimeInterval, excludeJobIds, false,false);
...
//					messageSender.sendSms(mobile, message);
	...
	public static CheckConcurrentRunningNumber getCheckConcurrentRunningNumber() {
		return BeanFactory.getBean(CheckConcurrentRunningNumber.class);
	}
