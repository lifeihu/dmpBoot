...
//   /home/tools/scheduler/scheduler_test jar   /home/tools/scheduler/scheduler_test.jar com.sw.bi.scheduler.background.javatype.precompute.PreComputeHour
@Component
public class PreComputeHour {
	...
	public static void main(String args[]){
		...
		PreComputeHour.PreComputeHour().computeHour();
		
	}
	
	
	private static PreComputeHour PreComputeHour() {
		return BeanFactory.getBean(PreComputeHour.class);
	}
	...
	public void computeHour(){
		...
		List<Job> jobs = jobService.getOnlineJobsByCycleType(Configure.JobCycle.HOUR.indexOf());
		for(Job job:jobs){
			...
			//主要是为了减少334,389,332这几个小时FTP任务在晚上的延迟告警
			long jobType = job.getJobType();
			if (Arrays.binarySearch(Parameters.FtpState, jobType) > -1) {
				cankao = 70*60;
			}
