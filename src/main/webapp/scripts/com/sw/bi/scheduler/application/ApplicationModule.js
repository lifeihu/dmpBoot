...
com.sw.bi.scheduler.application.ApplicationModule = Ext.extend(framework.core.Module, {
	...
	initModule: function() {
		...
		var oneTbar = new Ext.Toolbar({
			id: "g-tiptop",
			height: 45,
			items: this.createApplictionMenus()[0]
		});
		
		var twoTbar = new Ext.Toolbar({
			id: "g-tipbar",
			items: this.createApplictionMenus()[1]
		});
	...
	createApplictionMenus: function() {
		...
		if (!Ext.isEmpty(menus, false)) {
			...
			items2.push('您无权访问本系统!');
		}
		
		items2[0].iconCls = "menu-task224";
		
		return [items1, items2]; // new Ext.ButtonGroup({items: items});
	},
